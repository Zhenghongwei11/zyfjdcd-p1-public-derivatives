...
本地受限 Markdown 源文本为纸质《中医方剂大辞典》第 2 版的 OCR 衍生文本。除非后续取得明确授权或出版方许可，本项目默认不把整本 OCR 文本视为可公开再分发内容。

## 2. 发布分级

| tier | 名称 | 默认可见范围 | 当前默认状态 | 典型内容 |
| --- | --- | --- | --- | --- |
| `T0_PRIVATE_SOURCE` | 私有源文本 | 项目内部 | 允许 | 受限 OCR Markdown、原始截图、长文本片段 |
| `T1_RESTRICTED_EVIDENCE` | 受限证据材料 | 按需受控访问 | 条件允许 | 少量必要片段、困难样本截图、局部证据页 |
...
若期刊或编辑流程确需查看源文本，应按最小必要原则准备：

- 仅提供少量代表性片段
- 只提供与版次核验、困难样本说明、标注争议相关的必要证据
- 优先提供截图或局部片段，而不是整册文本
- 所有受限材料在 release manifest 中单列
